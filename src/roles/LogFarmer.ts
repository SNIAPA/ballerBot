...
export default class LogFarmer extends Role {
    ...
    constructor(options: LogFarmerOptions) {
        ...
        this.bedLocation = options.bedLocation
    }
    ...
    async execute() {
        try {
            while (this.bot.mBot) {
                ...
                try {
                    await this.bot.setSpawn(this.bedLocation)
    ...
    async deposit(chestLocation: Vec3) {
        ...
        console.log("done depositing, starting to withdraw")
